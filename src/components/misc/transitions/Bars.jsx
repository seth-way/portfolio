...
const Bars = ({ image, timing }) => {
		...
		setBars(prev =>
			prev.map(bar =>
				bar.animate.scaleY === 1.1
					? bar
					: { ...bar, animate: { scaleX: 1.1, scaleY: increment(bar.animate.scaleY) } }
			)
		);
	};
	...
	const revealAllBars = () =>
		setBars(prev => prev.map(bar => ({ ...bar, animate: { scaleX: 1.1, scaleY: 0 } })));
	const hideAllBars = () =>
		setBars(prev => prev.map(bar => ({ ...bar, animate: { scaleX: 1.1, scaleY: 1.1 } })));
			...
			<div className="absolute inset-0 grid grid-rows-2 grid-cols-48 select-none">
				{bars.map(item => (
					<motion.div
						...
						transition={{ type: 'spring', damping: 8, stiffness: 200, delay: item.delay }}
