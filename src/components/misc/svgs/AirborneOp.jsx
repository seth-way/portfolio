	...
	const addParatrooper = () => {
		if (isOpen && cardRef.current && airplaneRef.current) {
			...
			if (now - throttler.current > 200) {
				...
				const { left: planeLeft, right: planeRight } = airplaneRef.current.getBoundingClientRect();
	...
	return (
		<motion.div onClick={addParatrooper} ref={cardRef} className="absolute inset-0 z-20">
			<motion.h3
				...
				<span className={`${paratroopers.length ? 'text-primary_C' : 'text-gray-400'}`}>
					{paratroopers.length}
				</span>{' '}
				Troopers Deployed
			</motion.h3>
